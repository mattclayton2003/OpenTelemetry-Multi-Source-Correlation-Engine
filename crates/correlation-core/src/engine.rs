use crate::backend::{BackendError, LogQuery, SpanStatus, TelemetryBackend, TraceId};
...
impl Engine {
    ...
    pub async fn correlate_trace(
        ...
        let t_max = spans
            .iter()
            .map(|s| s.start + Duration::milliseconds(s.duration_ms))
            .max()
            .unwrap();
        ...
        Ok(IncidentContext {
            ...
            services: services
                .iter()
                .map(|name| ServiceSummary {
                    name: name.clone(),
                    span_count: spans.iter().filter(|s| s.service == *name).count(),
                    error_span_count: spans
                        .iter()
                        .filter(|s| s.service == *name && s.status == SpanStatus::Error)
                        .count(),
                    log_count: logs.iter().filter(|l| l.service == *name).count(),
                    error_log_count: logs
                        .iter()
                        .filter(|l| l.service == *name && l.level == "ERROR")
                        .count(),
                })
                .collect(),
            suspects: suspects
    ...
    pub async fn correlate_anomaly(
        ...
        use crate::anomaly::{zscore::ZScore, Detector};
        use crate::backend::AnomalyWindowQuery;
        let t0 = std::time::Instant::now();
        let pts = self
            .backend
            .query_metric_window(AnomalyWindowQuery {
                metric: metric.clone(),
        ...
        let det = ZScore {
            k: self.cfg.anomaly_zscore_k,
            min_baseline: self.cfg.min_baseline_sec as usize,
        };
        let hits = det.detect(&series);
        let mut notes = vec![];
        if hits.is_empty() {
            notes.push(format!(
                "no anomaly above threshold k={} in window",
                self.cfg.anomaly_zscore_k
            ));
            return Ok(self.empty_incident(
                Trigger::Anomaly {
...
fn build_tree(spans: &[crate::backend::Span]) -> Vec<TreeNode> {
    ...
    fn build(id: &str, children: &HashMap<Option<String>, Vec<String>>) -> TreeNode {
        ...
        TreeNode {
            span_id: id.into(),
            children: kids.iter().map(|c| build(c, children)).collect(),
        }
    }
    children
        .get(&None)
