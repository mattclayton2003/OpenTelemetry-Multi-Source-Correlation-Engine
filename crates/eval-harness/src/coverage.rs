...
pub struct AnomalyInvocation {
    #[serde(flatten)]
    pub classes: HashMap<String, InvocationEntry>,
}
#[derive(Debug, Deserialize)]
pub struct InvocationEntry {
    pub metric: String,
    pub service: String,
    pub window_pre_sec: i64,
    pub window_post_sec: i64,
}
...
pub fn anomaly_coverage(ic: &IncidentContext, expected_metrics: &[String]) -> f64 {
    ...
    let present: std::collections::HashSet<&str> = ic
        .metric_anomalies
        .iter()
        .map(|a| a.metric.as_str())
        .collect();
    let hit = expected_metrics
        .iter()
