...
pub struct SweepAnomaly {
    pub z_score_k: Vec<f64>,
    pub ewma_alpha: Vec<f64>,
}
#[derive(Debug, Deserialize)]
pub struct SweepRanking {
    pub causal_propagation_beta: Vec<f64>,
}
#[derive(Debug, Deserialize)]
pub struct SweepWindow {
    pub expansion_sec: Vec<i64>,
}
        ...
        for &a in &s.anomaly.ewma_alpha {
            for &b in &s.ranking.causal_propagation_beta {
                for &w in &s.window.expansion_sec {
                    out.push(correlation_core::CorrelationConfig {
                        anomaly_zscore_k: k,
                        anomaly_ewma_alpha: a,
                        causal_propagation_beta: b,
                        window_expansion_sec: w,
                        ..correlation_core::CorrelationConfig::default()
                    });
                }
            }
        }
