...
async fn correlate_trace(
    State(ctx): State<Ctx>,
    Json(req): Json<TraceReq>,
) -> Result<Json<IncidentContext>, (StatusCode, String)> {
    ctx.engine
        .correlate_trace(req.trace_id)
        .await
        .map(Json)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))
}
async fn correlate_anomaly(
    State(ctx): State<Ctx>,
    Json(req): Json<AnomalyReq>,
) -> Result<Json<IncidentContext>, (StatusCode, String)> {
    ctx.engine
        .correlate_anomaly(req.metric, req.service, req.start, req.end, req.value)
        .await
        .map(Json)
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))
}
...
async fn main() -> anyhow::Result<()> {
    let backend = MultiBackend {
        traces: Arc::new(correlation_tempo::TempoClient::new(
            std::env::var("TEMPO_URL").unwrap_or("http://tempo:3200".into()),
        )),
